...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.encoder = Encoder(
            [
                EncoderLayer(
                    AttentionLayer(
                        QuantumAttention(configs.d_model, configs.n_heads,
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        use_norm = getattr(self, "use_norm", True)

        if use_norm:
            # Non-stationary normalization (subtract mean, divide by std), matching the
            # de-normalization step at the end of this method. Previously this method
            # added back a mean it never subtracted, biasing every forecast upward.
            means = x_enc.mean(1, keepdim=True).detach()
            x_enc = x_enc - means
            stdev = torch.sqrt(torch.var(x_enc, dim=1, keepdim=True, unbiased=False) + 1e-5).detach()
            x_enc = x_enc / stdev

        # NOTE: __multi_scale_process_inputs computes down-sampled scales 1..S, but only
        # scale 0 (the original, finest resolution) is ever used below. Scales 1..S are
        # computed and discarded. This means the model does NOT currently implement the
        # multi-scale mechanism the paper's appendix analyzes for complexity -- either wire
        # the other scales into the encoder (e.g. average/concatenate their outputs) or
        # remove the multi-scale complexity derivation from the paper to match this code.
        ...
        if use_norm:
            # Revert the normalization applied above
    ...
    def anomaly_detection(self, x_enc):
        ...
        # Reuse the ORIGINAL stdev cached above, not a fresh var() computed on the
        # now-normalized x_enc (which would be ~1.0 and produce a ~40-50x scale error
        # in dec_out -- confirmed numerically before this fix).
        dec_out = dec_out * stdev + means
        return dec_out
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        if self.task_name in ['long_term_forecast', 'short_term_forecast']:
            ...
            raise NotImplementedError(
                "QISARTF does not currently implement the 'imputation' task. "
                "This branch previously called a non-existent self.imputation(...) method."
            )
